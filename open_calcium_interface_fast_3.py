...
if __name__ == "__main__":
    ...
    encoding = 'fast'
    filetype = 3 # 1 = rows-columns EWMA csv file; 2 = rows-columns RAWintDen csv file; 3 = FIJI ROI csv file    
    timestamp_file = 1
    ...
    if filetype == 1:
        ...
        delta_f = calc_delta_f(cells_lopass, cells_lopass) #2nd arg is the source for f0(will take mean, list size (#elements) must match 1st arg)
        cells = delta_f
        run_lopass = 1        
    elif filetype == 3:
        intensity_data = Data.open_intensity_csv()
        cells = Data.get_cells(intensity_data)
        delta_f = calc_delta_f(cells, cells)
        run_lopass = 1
    ...
    if encoding == 'slow':
        ...
        # get frame indices for flip and flop onsets        
...
gray_session_onset_i = 1 + (sorted(flop_i)[-1][1]+(sorted(flop_i)[-1][1] - sorted(flop_i)[-2][1]))  

    
# Chunk up GRAY SESSION means and adjust lengths of sessions so equal
...
#==============================================================================
# Chunk up STIM SESSION means of truncated lsist
session_avgs = list()
for cell in sessions:
...
#==============================================================================
#==============================================================================            
# Combine GRAY and STIM average DATA 
...
# Plot each trace and mean on one figure
nrows = 17
ncols = 4
plotSessionTraces(nrows, ncols, stimsession_cell, session_avgs)

plotSessionTraces(nrows, ncols, sessions_stimgray, session_gray_avgs, gray_session_onset_i)
...
for i in range(len(flip_i)):
    ...
    plt.axvline(x=gray_session_onset_i, ymin=0, ymax = len(session_gray_avgs), linewidth=0.5, color='k')
